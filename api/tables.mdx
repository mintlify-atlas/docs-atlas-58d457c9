...
<Note>
  The `tiene_pedido_activo` boolean flag is only present on responses from `GET /api/tables`. It is **not** returned by `GET /api/tables/:id` or `GET /api/tables/disponibles`. See the [GET /api/tables](#get-apitables) section for details.
</Note>
