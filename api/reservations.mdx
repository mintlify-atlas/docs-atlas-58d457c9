...
description: "Manage table reservations: create, confirm, check availability by time window, and convert bookings into active orders. Includes a public booking route."
---
...
Accepts a booking request from a customer without requiring an account. The server always creates a new customer record from the supplied contact details and assigns the most suitable available table if `mesa_id` is not specified. No de-duplication is performed on the customer data.
