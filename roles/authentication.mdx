---
title: "Authenticate with SS Restaurant API"
...
<Steps>
  ...
  <Step title="Handle expired or invalid tokens">
    When the API returns a non-OK response, `apiFetch` inspects the status code. A `401` response automatically clears both `localStorage` keys and redirects the user to `/login`. Your components do not need to handle token expiry explicitly — the utility takes care of it.
  </Step>
</Steps>
...
| `descripcion` | `Inicio de sesion: <email>` |
| `ip` | Client IP address (resolved via `req.ip`, with `trust proxy` enabled) |
